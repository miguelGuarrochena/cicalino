...
/* Lotes por corrida del cron. Cada lote manda mails de a uno (Resend); el
 * techo evita timeout de Vercel. Varios lotes cubren >200 orgs accionables. */
const MAX_ORGS_POR_LOTE = 200;
const MAX_LOTES = 10;
...
export const sweepSubscriptions = async (): Promise<{
  ...
  let lote = 0;
  let quedaronSinRevisar = false;

  while (lote < MAX_LOTES) {
    lote++;
    /* Solo orgs sobre las que planDailyActions puede actuar. Nulls de plan/
     * estado se tratan abajo como mensual/active (accionables). */
    let q = admin
      .from("organizaciones")
      .select(
      ...
      .order("id")
      .limit(MAX_ORGS_POR_LOTE);
    if (cursor) q = q.gt("id", cursor);

    const { data, error } = await q;
    if (error || !data) {
      console.error("sweepSubscriptions", error?.message);
      return { ok: false, revisadas, mails, cambios };
    }
    if (data.length === 0) break;

    cursor = data[data.length - 1]!.id;
    revisadas += data.length;
    if (data.length === MAX_ORGS_POR_LOTE && lote === MAX_LOTES) {
      quedaronSinRevisar = true;
    }
    ...
    if (data.length < MAX_ORGS_POR_LOTE) break;
  }

  if (quedaronSinRevisar) {
    console.error(
      `sweepSubscriptions: se alcanzó el techo de ${MAX_LOTES * MAX_ORGS_POR_LOTE} organizaciones en una corrida. Quedaron sin revisar.`,
    );
  }

  return { ok: true, revisadas, mails, cambios };
};
