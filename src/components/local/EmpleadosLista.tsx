...
export const EmpleadoModal = ({ onClose }: { onClose: () => void }) => {
  const { t } = useApp();
  const empleados = useConfigStore((s) => s.empleados);
  const agregarEmpleado = useConfigStore((s) => s.agregarEmpleado);
  ...
  const validar = (): FieldErrors => {
        ...
        if (!nombre.trim()) next.nombre = t("config.empNombreReq");
        if (!isPin4(pin)) next.pin = t("config.empPinReq");
        else if (pinEnUso(pin, empleados)) next.pin = t("config.empPinDup");
        return next;
      };
  ...
  const guardar = () => {
        ...
        agregarEmpleado({ nombre, rol, pin });
        onClose();
      };
...
export const EmpleadosLista = () => {
  const { t } = useApp();
  const empleados = useConfigStore((s) => s.empleados);
  const quitarEmpleado = useConfigStore((s) => s.quitarEmpleado);
  const [modal, setModal] = useState(false);
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [page, setPage] = useState(1);

  const pageItems = slicePage(empleados, page, PAGE_SIZE);
  ...
  return (
    <>
                  ...
                  <div className="flex shrink-0 flex-col gap-1.5 sm:flex-row sm:items-center">
                    <button
                      ...
                      onClick={() => {
                        quitarEmpleado(e.id);
                        setConfirmId(null);
                        const remaining = empleados.length - 1;
      ...
      {modal && <EmpleadoModal onClose={() => setModal(false)} />}
    </>
  );
};
