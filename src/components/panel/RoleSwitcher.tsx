...
const ROLES_LOCAL: CurrentRole[] = ["admin", "supervisor", "empleado"];

// Selector de rol del LOCAL para previsualizar cada vista en el prototipo.
// Superadmin vive aparte en /admin y se entra desde /login.
// En produccion el rol viene del login y esto no existe.
export const RoleSwitcher = () => {
  const { locale, t } = useApp();
  const { rol: role, setRol: setRole, impersonando: impersonating, salirImpersonacion: exitImpersonation } = useSessionStore();
  const [open, setOpen] = useState(false);
  const path = usePathname();
  const router = useRouter();
  const enAdmin = path.startsWith("/admin");

  const label = (r: CurrentRole) => {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="hidden sm:inline">{label(role)}</span>
      </button>
      {open && (
        <>
            ...
            {ROLES_LOCAL.map((r) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-xl px-2.5 py-2 text-left text-sm transition hover:bg-carbon/5 ${
                  role === r ? "font-semibold text-marca" : "text-carbon"
                }`}
              >
                {label(r)}
                {role === r && <span>✓</span>}
