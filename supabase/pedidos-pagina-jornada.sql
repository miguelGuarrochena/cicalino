-- ===========================================================================
-- Cicalino — pedidos_pagina usa la jornada de Postgres, no la del cliente
-- Correr en: Supabase Dashboard → SQL Editor / pnpm db:sql. Idempotente.
-- Requiere: pedidos-paginado.sql, security-fixes-15.sql (hora_corte TZ AR)
-- Orden: al final de orden.json
--
-- PROBLEMA
-- crear_pedido calcula la jornada con locales.hora_corte en TZ Argentina e
-- ignora p_desde. pedidos_pagina filtraba con el p_desde que mandaba el
-- navegador (Zustand, default 6 hasta hidratar). Si el corte del local no
-- era 6, o el store todavía no había cargado, el pedido existía en DB y no
-- aparecía en el listado — o el próximo número no coincidía con el alta.
--
-- FIX
-- Misma cuenta que crear_pedido. p_desde se queda en la firma para no romper
-- PostgREST; el servidor no lo usa.
-- ===========================================================================
